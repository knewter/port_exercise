...
Elm.Main.make = function (_elm) {
   ...
   var outboundMailbox = $Signal.mailbox(0);
   var outbound = Elm.Native.Port.make(_elm).outboundSignal("outbound",function (v) {    return v;},outboundMailbox.signal);
   var initialModel = 0;
   var NoOp = {ctor: "NoOp"};
   var sendToOutboundMailbox = function (count) {
      return A2($Effects.map,$Basics.always(NoOp),$Effects.task(A2($Signal.send,outboundMailbox.address,count)));
   };
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "Increment": return {ctor: "_Tuple2",_0: model + 1,_1: $Effects.none};
         case "Send": return {ctor: "_Tuple2",_0: model,_1: sendToOutboundMailbox(model)};
   ...
   var mapIncrement = function (_p1) {    return Increment;};
   var app = $StartApp.start({init: {ctor: "_Tuple2",_0: initialModel,_1: $Effects.none}
                             ,update: update
                             ,view: view
                             ,inputs: _U.list([A2($Signal.map,mapIncrement,increment)])});
   var main = app.html;
   ...
   return _elm.Main.values = {_op: _op
                             ...
                             ,sendToOutboundMailbox: sendToOutboundMailbox};
};
